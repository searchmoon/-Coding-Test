...
//이진수로 변환했을때, 1의 개수가 다음 숫자와 같아야함.
// 이진수로 변환하는 함수 하나 만들고, n을 넣었을때와 n+1을 넣었을때의 반환값을 비교한 후 같으면 return

//재사용되는 부분 함수로 빼주기
function countOneLength (num) {
    return num.toString(2).split("").filter(bit => bit === "1").length;
}

function solution(n) {
    const originalCount = countOneLength(n);
    let nextNumber = n + 1;
    
    while(countOneLength(nextNumber) !== originalCount){
        nextNumber++;
    }
    return nextNumber;
}

// //이진수로 변환하기: parseInt(num, 2), toString(2)
